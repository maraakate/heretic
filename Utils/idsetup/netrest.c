...
int RestoreNetwork(void)
{
	...
	char  p1,p2,p3,p4;
#ifdef HEXEN
	char p5,p6,p7,p8; // FS: Hexen has 8 players
#endif
	char	versiontest[7]; // FS: Find out if this is an extended port
	char	tempstring[10];
	...
	for (i = 0;i < 6;i++)
	{
...
#ifdef HEXEN
		lseek(handle,27+531,SEEK_SET); // FS: Hexen's playeringame is a lot farther in
#else
		lseek(handle, 24, SEEK_SET);
		read(handle,versiontest,7);

		if(versiontest[0] == 'v') // FS: Test to see if this is an extended port
			lseek(handle,27+16,SEEK_SET);
		else
			lseek(handle,259+16,SEEK_SET); // FS: Extended 256 stringsizelimit
		...
		numplayers[i] = p1+p2+p3+p4+p5+p6+p7+p8;
#else
